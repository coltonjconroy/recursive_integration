...
if strcmpi(month,'5')
    p_start = 1;
    U       = ndbc_45005_2010(p_start:(p_start+31*24),7);
    ubad    = find(U == 99.0);
    U(ubad) = U(ubad+1)+U(ubad-1)./2;
    theta_w = ndbc_45005_2010(p_start:(p_start+31*24),6);
...
elseif strcmpi(month,'6')
    p_start = 24*31+1;
    U       = ndbc_45005_2010(p_start:(p_start+30*24),7);
    ubad    = find(U == 99.0);
    U(ubad) = U(ubad+1)+U(ubad-1)./2;
    theta_w = ndbc_45005_2010(p_start:(p_start+30*24),6);
    tbad    = find(theta_w == 99.0);
    theta_w(tbad) = theta_w(tbad+1)+theta_w(tbad-1)./2;
    N  = 24*30;
    Lw = N+1; % # of mixing nodes
